# 반복자 <small>(Iterator)</small> 패턴

> 내부 표현부를 노출하지 않고 어떤 객체 집합에 속한 원소들을 순차적으로 접근할 수 있는 방법을 제공하는 패턴

> 반복자 패턴의 아이디어는 객체가 데이터를 저장하는 방법과 이 데이터를 순회하는 방법을 분리하는 것이다. <small>(집합 객체 단순화)</small>

## 구조

<p align = 'center'>
<img width = '600' src = 'https://user-images.githubusercontent.com/39554623/150173513-e38f86e0-5482-4d09-9ae7-b2679cc3969b.png'>
</p>

- **Iterator**: 원소를 접근하고 순회하는 데 필요한 인터페이스를 제공한다.
- **ConcreteIterator**: Iterator에 정의된 인터페이스를 구현하는 클래스로, 순회 과정 중 집합 객체 내에서 현재 위치를 기억한다.
- **Aggregate**: Iterator 객체를 생성하는 인터페이스를 정의한다. <small>(Aggreate는 '집합'이라는 뜻을 가지고 있음)</small>
...
- 반복자 패턴은 복합체<small>(컴포지트, composite)</small> 패턴과 같이 재귀적 구조가 있을 때 자주 사용한다.
...
보병<small>(InfantryUnit)</small>으로 구성된 분대<small>(Squad)</small>가 있다.
...
각 분대는 지휘관이 필요하다. 각 분대의 지휘관으로 보병 유닛인 병장<small>(Sergeant)</small>을 추가한다.
...
소대<small>(Platoon)</small>는 분대의 집합이며 중위<small>(Lieutenant)</small>가 지휘관이다.
...
<p align = 'center'>
<img width = '500' src = 'https://user-images.githubusercontent.com/39554623/150164172-5f3ab9d1-ab18-439f-9d1f-3492fa5c35f5.png'>
</p>
...
먼저 반복자<small>(iterator)</small>를 위한 상태가 필요하다. 이는 마지막 요소가 반환되었음을 기억해준다.
...
마지막으로 반환할 유닛을 알아야 한다. 만약 첫 번째 호출이라면 지휘관인 병장<small>(sergeant)</small>를 반환할 것이고, 그 다음부턴 분대의 멤버들을 반환할 것이다.
