# 팩토리 메서드<small>(Factory Method)</small> 패턴
...
이것은 아주 간단한 예시이다. 객체<small>(이 경우, `Animal`)</small>에 대한 공통 인터페이스를 제공한 다음 몇 가지 인자를 기반으로 인스턴스화할 구체적인 클래스를 결정하였는데, 추가적으로 다른 품종을 지원한다면 어떻게 해야 할까?
